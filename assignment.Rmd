...
Utilizing KNN requires a different setup in how the data is provided to `class::KNN` than we saw with `MASS::lda`. Recycling  the training, validation and test split from before we simply modify the structure to facilitate using our next method.
...
Based on this output, it appears that the best choice as far this KNN model goes is to utilize $k=7$. Doing so with the test data set achieves the following accuracy. 

```{r}
mean(do_knn(knn_data[["test"]], 7) == knn_data[["test"]]$Loan_Status)
```
