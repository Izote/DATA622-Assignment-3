...
training <- slice_sample(loans_lda, prop = 0.70)

remaining <- anti_join(loans_lda, training, by = "Loan_ID")
...
```{r}
lda_mod <- MASS::lda(
  Loan_Status ~ ApplicantIncome + LoanAmount, data = training
  )
```

```{r}
lda_preds <- predict(lda_mod, newdata = validation)

validation <- validation %>% 
  mutate(
    Posterior = lda_preds$posterior[, "N"],
    Predicted_Status = if_else(Posterior <= 0.27, "N", "Y"),
    Accuracy = Loan_Status == Predicted_Status
  )
```

```{r}
mean(validation$Accuracy, na.rm = TRUE)
```
